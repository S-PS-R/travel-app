...
export default function TransportDetails({leg,onChange,disabled}:{leg:TravelLeg;onChange:(leg:TravelLeg)=>void;disabled:boolean}) {
  ...
  return <View style={{gap:12}}>
    ...
    {mode==="flight"&&<Text style={s.muted}>Use the full flight number and departure date to identify the flight, for example AI 101. Times are local to departure.</Text>}
    ...
    {mode==="flight"&&<FlightLookup key={`${leg.fromId}:${leg.toId}:${details.serviceNumber}:${details.departureDate}`} number={details.serviceNumber??""} date={details.departureDate??""} disabled={disabled} onUse={flight=>onChange({...leg,flight,details:{...leg.details,flight:{...details,operator:flight.airline,serviceNumber:flight.number,departureDate:flight.departureDate,departureTime:flight.departureTime,fromStation:flight.departureAirport,toStation:flight.arrivalAirport}}})}/>}
